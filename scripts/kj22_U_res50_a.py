...
save_dir= './kj21_d_mask'
# mode='eval'
mode='train'
model_pretrained_name= os.path.join('./kj21_d_mask' , '_ep_mse_loss_7.model')
...
df_train, df_dev = train_test_split(train, test_size=0.001, random_state=42)
df_test = test
...
class single_conv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, Maxpool=True):
        ...
        if(Maxpool):
            self.conv.add_module('Max-pool',nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
...
class UNetWithResnet50Encoder(nn.Module):
    ...
    def __init__(self, n_classes=2):
        ...
        self.out  = single_conv(64 , 256)
        self.out2 = single_conv(256, 512)
        
        self.mask = nn.Sequential ( single_conv(1024, 512 ,Maxpool =False) , \
                                    single_conv(512, 256 ,Maxpool =False)  , \
                                    single_conv(256, 64 ,Maxpool =False)  , \
                                    single_conv(64, 1 ,Maxpool =False))
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        x = self.out  (x)
        x = self.out2 (x)
        print('self.out2 --> ',x.shape)
        
        x = torch.cat([xx[2] , x_block , x] , 1)
        
        
        x = self.mask(x)
        # x = self.out(x)
        print('7- '.format(i),x.shape)
        del pre_pools
...
# print("{} Model is loaded.".format(model_pretrained_name))
# evaluate_model(1)
...
for ind , (name ,param) in enumerate(model.named_parameters()):
    print(ind , '  ' ,name , param.requires_grad )
...
if(mode == 'train'): 
    ...
    for epoch in range(n_epochs):
        ...
        torch.save(model.state_dict(), os.path.join(save_dir, '_ep_mse_loss_{}.model'.format(epoch)))
