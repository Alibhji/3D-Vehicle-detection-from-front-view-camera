...
class MyUNet(nn.Module):
    ...
    def __init__(self, n_classes):
        super(MyUNet, self).__init__()
        self.base_model=pretrainedmodels.__dict__['pnasnet5large'](num_classes=1000, pretrained='imagenet')  # size --> 345153926
    ...
    def forward(self, x):
        print('Input  ', x.shape)
        x = self.base_model.conv_0(x)
        print('conv_0  ', x.shape)
        x = self.base_model.cell_stem_0(x)
        print('Cell_stem_0  ', x.shape)
        x = self.base_model.cell_stem_1 (x)
        print('Cell_stem_1 ', x.shape)
