...
def preprocess_image(img, flip=False):
    img = img[:(img.shape[0] - CUT_from_TOP) , : (img.shape[1] - CUT_from_RIGHT)]
...
def get_mask_and_regr(img, labels, flip=False):
    mask = np.zeros([(ORG_IMG_HEIGHT-CUT_from_TOP) // MASK_SCALE, (ORG_IMG_WIDTH-CUT_from_RIGHT) // MASK_SCALE], dtype='float32')
#     regr_names = ['x', 'y', 'z', 'yaw', 'pitch', 'roll']
    regr_names = ['yaw', 'pitch', 'roll']
#     regr = np.zeros([(ORG_IMG_HEIGHT-CUT_from_TOP) // MASK_SCALE, (ORG_IMG_WIDTH-CUT_from_RIGHT) // MASK_SCALE ,7], dtype='float32')
    regr = np.zeros([(ORG_IMG_HEIGHT-CUT_from_TOP) // MASK_SCALE, (ORG_IMG_WIDTH-CUT_from_RIGHT) // MASK_SCALE ,4], dtype='float32')
...
#         print(x,y)
        ...
        y = (y - CUT_from_TOP ) / MASK_SCALE
        y = np.round(y).astype('int')
#         print(x,y , ORG_IMG_WIDTH // MASK_SCALE , ORG_IMG_HEIGHT // MASK_SCALE)
        if x >= 0 and x < ORG_IMG_HEIGHT // MASK_SCALE and y >= 0 and y < ORG_IMG_WIDTH // MASK_SCALE:
            mask[x, y] = 1
...
class CarDataset(Dataset ):
    ...
    def __init__(self, dataframe, root_dir, training=True, transform=None , skip_masks=False):
        self.df = dataframe
...
mode='eval'
#mode='train'
model_pretrained_name= os.path.join(save_dir , '_ep_11.model')
...
print('train_loader_All --> Dataset Len-->',len(train_loader_All.dataset))

i__=0
predict_dict_res=OrderedDict()
    
for img, names in tqdm(train_loader_All):
    with torch.no_grad():
...
test.to_csv('predictions_train_kj11.csv', index=False)
test.head()
with open(os.path.join(save_dir, 'predictions_train_kj11.pkl.pandas'), 'wb') as handle:
    pickle.dump(test, handle, protocol=pickle.HIGHEST_PROTOCOL)
print("predicion is done & csv file is saved.")

print('The length of predicted tensors:',len(predict_dict_res))
save_file_name= 'prediction_on_train_dataset_Tensors_kj11_.pkl'
    #with open(os.path.join('./', 'train_prediction_{}.pkl'.format(model_name)), 'wb') as handle:
with open(os.path.join('./', save_file_name), 'wb') as handle:
